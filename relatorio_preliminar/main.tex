...
\textit{DevOps} é um conjunto de filosofias e práticas que promovem o desenvolvimento e lançamento de aplicações com maior rapidez e superior qualidade. Duas das práticas mais relevantes em \textit{DevOps} são \textit{Continuous Integration} e \textit{Continuous Delivery}. \textit{Continuous Integration} é uma prática aonde programadores integram, com regularidade, código desenvolvido para um repositório central aonde é automaticamente compilado e são efetuados testes sobre o mesmo. Este mesmo código é automaticamente preparado para lançamento, sendo a base de \textit{Continuous Delivery}. Com o objetivo de promover estas práticas a empresa de ARTSOFT usa a ferramenta Jenkins. Jenkins é um servidor de automação \textit{open source} que corre em \textit{servlet containers}. Jenkins facilita \textit{Continuous Integration} e \textit{Continuous Delivery} utilizando \textit{pipelines} para automizar a compilação de binários. Ao definir um conjunto de processos, pipelines permitem a compilação, contrução e lançamento automático de código.
